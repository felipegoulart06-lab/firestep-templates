  ...
  <main class="product-page container">
    ...
    <article class="product-layout" itemscope itemtype="https://schema.org/Product">
      <div class="product-main">
        ...
        ${hero ? `<figure class="product-hero"><button type="button" class="gallery-zoom" data-lightbox="${escapeHtml(pageAsset(hero))}" aria-label="Ampliar preview"><img src="${escapeHtml(pageAsset(hero))}" alt="Preview do template ${escapeHtml(template.name || "")}" itemprop="image" width="1200" height="750" data-lightbox="${escapeHtml(pageAsset(hero))}"></button></figure>` : ""}
        ${extraImages.length ? `<div class="product-gallery">${extraImages.map((url, index) => `<figure data-lightbox="${escapeHtml(pageAsset(url))}"><button type="button" class="gallery-zoom" data-lightbox="${escapeHtml(pageAsset(url))}" aria-label="Ampliar imagem ${index + 2}"><img src="${escapeHtml(pageAsset(url))}" alt="Imagem ${index + 2} do template ${escapeHtml(template.name || "")}" data-lightbox="${escapeHtml(pageAsset(url))}"></button></figure>`).join("")}</div>` : ""}
